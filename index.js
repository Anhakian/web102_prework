...
const individualContributions = GAMES_JSON.reduce( (sum, game) => {return sum + game.backers}, 0 );
...
function filterUnfundedOnly() {
    ...
    const unfundedGames = GAMES_JSON.filter( (game) => {return game.goal > game.pledged} );
...
const countUnfunded = GAMES_JSON.reduce( (acc, game) => {
    return game.goal > game.pledged ? acc + 1 : acc
}, 0);

// create a string that explains the number of unfunded games using the ternary operator
const display = `$${totalRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games.
Currently, ${countUnfunded} game${countUnfunded != 1 ? 's are' : ' is'} unfunded.`;
